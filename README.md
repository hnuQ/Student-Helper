...
当前 Release 主要提供 Windows 安装包。

1. 打开 [Releases](https://github.com/hnuQ/Student-Helper/releases) 页面。
2. 在最新版本的 Assets 中下载 Windows 安装包，例如 `Student-Helper-2.1.1-win-setup.exe`。
3. 双击 `.exe` 文件启动安装向导，并按提示完成安装。
4. 安装完成后，在开始菜单或桌面找到 **Student Helper**，双击运行。
...
- macOS 和 Linux 打包配置已保留在项目中，但当前 README 只按 Windows 安装包说明使用方式。
...
构建产物会输出到 `release/` 目录，Windows 安装包文件名类似：

```text
Student-Helper-2.1.1-win-setup.exe
```
